import 'dart:ui';
import 'package:ai_scheduler/features/assistant/cubit/assistant_cubit.dart' show AssistantCubit, AssistantState;
...
class _VoiceInputScreenState extends State<VoiceInputScreen> {
  ...
  void initState() {
    ...
    context.read<AssistantCubit>().startVoiceRecording();
  }

  @override
  Widget build(BuildContext context) {
    return _VoiceInputScreenView();
  }
}

class _VoiceInputScreenView extends StatefulWidget {
  const _VoiceInputScreenView({super.key});

  @override
  State<_VoiceInputScreenView> createState() => _VoiceInputScreenViewState();
}

class _VoiceInputScreenViewState extends State<_VoiceInputScreenView> with SingleTickerProviderStateMixin {
        ...
        child: Container(
          ...
          child: Stack(
            children: [
              ...
              // Center content
              Center(
                child: BlocSelector<AssistantCubit, AssistantState, bool>(
                  selector: (state) {
                        ...
                        Stack(
                          ...
                          children: [
                              ...
                              builder: (context, child) {
                                return Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(width: 2, color: const Color(0xFF8B5CF6).withOpacity(0.3)),
                                  ),
                                );
                              },
                            ),
                            ...
                            GestureDetector(
                              onTap: () {
                                if (recording) {
                                  context.read<AssistantCubit>().stopVoiceRecording();
                                } else {
                                  context.read<AssistantCubit>().startVoiceRecording();
                                }
                              },
                        ...
                        Container(
                          ...
                          child: AnimatedBuilder(
                            ...
                            builder: (context, child) {
                              ...
                              return Opacity(opacity: calculatedOpacity.clamp(0.0, 1.0), child: child);
                            },
                            child: Text(
                              recording
                                  ? 'Щойно завершите, натисніть на\nмікрофон знову'
                                  : 'Натисніть на анімацію щоб надавати команди асистенту',
...
class _AssistantAnimation extends StatefulWidget {
  const _AssistantAnimation({super.key});

  @override
  State<_AssistantAnimation> createState() => _AssistantAnimationState();
}

class _AssistantAnimationState extends State<_AssistantAnimation> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _voiceController = AnimationController(duration: const Duration(milliseconds: 1500), vsync: this)..repeat();
    ...
    _dot2Animation = TweenSequence<double>([
      // Wait for dot 1 (0-25%)
      TweenSequenceItem(tween: ConstantTween<double>(0.0), weight: 25.0),
      // Jump (25-50%)
      TweenSequenceItem(
        tween: Tween<double>(begin: 0.0, end: -16.0).chain(CurveTween(curve: Curves.easeOut)),
        weight: 12.5,
